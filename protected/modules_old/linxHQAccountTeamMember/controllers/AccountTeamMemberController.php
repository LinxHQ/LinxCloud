...
class AccountTeamMemberController extends LinxHQController {
    ...
    public function actionCreate() {
        $model = new LinxHQAccountTeamMember;

        // Uncomment the following line if AJAX validation is needed
        // $this->performAjaxValidation($model);

        if (isset($_POST['LinxHQAccountTeamMember'])) {
            $model->attributes = $_POST['LinxHQAccountTeamMember'];
            if ($model->save())
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('LinxHQAccountTeamMember');
        $this->render('team.index', array(
    ...
    public function actionDropdownSource() {
        if (isset($_GET['account_id'])) {
            ...
            if (!LinxHQAccountTeamMember::model()->isValidMember($master_account_id, Yii::app()->user->id) && Yii::app()->user->id != $master_account_id) {
                return '';
            }

            $account_team_members = LinxHQAccountTeamMember::model()->getTeamMembers($master_account_id, true);
            ...
            foreach ($account_team_members as $m) {
                ...
                //	continue;
                $name = LinxHQAccountProfile::model()->getShortFullName($m->member_account_id);
            ...
            $master_name = LinxHQAccountProfile::model()->getShortFullName($master_account_id);
    ...
    public function actionAdmin() {
        ...
        $memberCADataProvider = LinxHQAccountTeamMember::model()->getTeamMembers(Yii::app()->user->id);

        // CADP for getting list of members from OTHER teams of which user is NOT the master account of.
        $otherMemberCADataProvider = LinxHQAccountTeamMember::model()->getMyOtherTeams(Yii::app()->user->id);

        // CADP for invites to this user
        $invitesToUserCADataProvider = LinxHQAccountInvitation::model()->getInvitesToAccount(Yii::app()->user->id);

        // get master account of people whose teams I'm part of
        $model2 = new LinxHQAccountTeamMember('search');
        $model2->unsetAttributes();
        $model2->member_account_id = Yii::app()->user->id;

        if (isset($_GET['LinxHQAccountTeamMember']))
            $model->attributes = $_GET['LinxHQAccountTeamMember'];

        LinxHQApplication::render($this, 'team.admin', array(
    ...
    public function loadModel($id) {
        $model = LinxHQAccountTeamMember::model()->findByPk($id);
        if ($model === null)
