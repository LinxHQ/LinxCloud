...
class LinxHQAccount extends LinxHQCActiveModel
{
	...
	public $account_new_password_retype;
        ...
        // additional object for registration form validation
        public $accountProfile = null;
        public $accountSubscription = null;
	...
	**/
	 ...
	 * @return LinxHQAccount the static model class
	 */
	...
	public function relations()
	{
		...
		return array(
			'account_member' => array(self::HAS_MANY, 'LinxHQAccountTeamMember', 'account_id'),
			'account_profile' => array(self::HAS_ONE, 'LinxHQAccountProfile', 'account_id'),
	...
	public function save($runValidation=true, $attributes=NULL) {
		...
		if ($this->isNewRecord) {
                        // if additional form data present (subscription, profile)
                        // must validate
                        if ($this->accountSubscription)
                        {
                            if ($this->accountSubscription->application_id == null)
                            {
                                $this->addError('', 'Please choose an application');
                                        return false;
                            }
                            if ($this->accountSubscription->account_subscription_package_id == null)
                            {
                                $this->addError('', 'Please choose a subscription package');
                                return false;
                            }
                        } // end check aaccount subscription
                        if ($this->accountProfile)
                        {
                            if ($this->accountProfile->account_profile_surname == null
                                    || $this->accountProfile->account_profile_given_name == null)
                            {
                                $this->addError('', 'Please fill in *both* Given Name and Surname.');
                                        return false;
                            }
                        }// end check account profile
	...
	**/
	...
	{
		$account = LinxHQAccount::model()->find('account_email = :account_email', 
	...
	public function isMasterAccount($subscription_id, $account_id = 0)
	{
		...
		$subscription = LinxHQAccountSubscription::model()->find('account_subscription_id = :account_subscription_id AND
	...
	{
		if (LinxHQApplication::isValidEmail($this->account_email) !== false)
		{
			// find if this email is used by other account already or not
			$account = LinxHQAccount::model()->find('account_id != :account_id AND account_email = :account_email', array(
